...
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
		...
		private TimeSpan GetNoteLengthInTimespan(MarkablePlaybackEvent markablePlaybackEvent)
		{
			TimeSpan noteTime = default(TimeSpan);
			if (markablePlaybackEvent != null && markablePlaybackEvent.RelatedEvents.Count > 0)
			{
				...
				{
					noteTime = noteOff.Time < noteOn.Time ? noteOff.Time : noteOn.Time;
				}
				else if (noteOff != null)
				{
					noteTime = noteOff.Time;
				}
				else if (noteOn != null)
				{
					noteTime = noteOn.Time;
				}

				return noteTime - markablePlaybackEvent.Time;
			}
